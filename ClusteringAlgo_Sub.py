...
class AUL_Clustering:
    ...
    def runWithoutSubsampling(self, mode, algo=""):
        if os.path.exists("ClusteringOutput/"+self.fileName+"_"+self.algoName+".csv"):
            ...
            if self.X == []:
                self.readData()
        t0 = time.time()
        p = multiprocessing.Process(target=self.runWithoutSubsampling_w, args=(mode,algo,))
        p.start()
    ...
    def runWithoutSubsampling_w(self, mode,algo):
        ...
        if mode == "default":
            if self.algoName == "AP":
                ...
                c = DBSCAN(algorithm=algo).fit(self.X)
                l = c.labels_
