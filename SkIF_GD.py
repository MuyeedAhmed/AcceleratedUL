...
def isolationforest(filename, parameters, parameter_iteration):
    ...
    # X = pd.read_csv(folderpath+filename+".csv")
    # for i in range(10):
    #     X_s = X.sample(frac=0.1, replace=True, random_state=i)
    #     gt=X_s["target"].to_numpy()
    #     X_s=X_s.drop("target", axis=1)
        
        
    #     blind_route = get_blind_route(X_s, gt, filename, deepcopy(mod_parameters), parameter_iteration)
    ...
    # t1 = time.time()
    # print("Subsampleing time: ")
    # print(t1-t0)
    
    
    # param=[]
    # param.append(["n_estimators", 100, []])
    # param.append(["max_samples", 'auto', []])
    # param.append(["contamination", 'auto', []])
    # param.append(["max_features", 0.9, []])
    # param.append(["bootstrap", False, []])
    # param.append(["n_jobs", 1, []])
    # param.append(["warm_start", True, []])
    
    # default_f1, default_ari = runIF(filename, X, gt, param, 0)
    # print(default_f1, default_ari)
    # return
    withGT=False
...
def get_blind_route(X, gt, filename, parameters_this_file, parameter_iteration):
    ...
    for p_i in range(len(parameters_this_file)):
        ...
        while True:
            ...
            if i_pv < 0:
                break
            X_s = X.sample(frac=0.1, replace=True, random_state=p_i)
            passing_param[p][1] = parameters_this_file[p][2][i_pv]
            f1_score, ari_score = runIF(filename, X_s, gt, passing_param, parameter_iteration)
...
if __name__ == '__main__':
    ...
    n_estimators = [2, 4, 8, 16, 32, 64, 100, 200, 350, 500]##
