...
if __name__ == '__main__':
    ...
    if os.path.exists("Stats/"+algorithm+"_Incomplete.csv"):
        done_files = pd.read_csv("Stats/"+algorithm+"_Incomplete.csv")
        done_files = done_files["Filename"].to_numpy()
        master_files = [x for x in master_files if x not in done_files]
    
    master_files.sort()
        
    if os.path.exists("Stats/"+algorithm+"_Incomplete.csv") == 0:
        f=open("Stats/"+algorithm+"_Incomplete.csv", "w")
